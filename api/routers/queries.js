...
router.get('/features/:itemId', async (req, res) => {
    try {
        const { itemId } = req.params;
        const [features] = await connection.query(
            'SELECT FeatureKey, FeatureValue FROM ProductFeatures WHERE ItemID = ?', [itemId]
        );
        res.json(features);
    } catch (error) {
...
router.get('/filters/:itemId', async (req, res) => {
    try {
        const { itemId } = req.params;
        const [filters] = await connection.query(
            'SELECT FilterKey, FilterValue FROM ProductFilters WHERE ItemID = ?', [itemId]
        );
        res.json(filters);
    } catch (error) {
...
router.get('/products', async (req, res) => {
    try {
        const { itemId, RAM, Color, Storage } = req.query;
        
        let query = `SELECT * FROM Inventory WHERE ItemID = ${itemId}`;
        
        if (RAM) query += ` AND ItemID IN (SELECT ItemID FROM ProductFilters WHERE FilterKey = 'RAM' AND FilterValue = '${RAM}')`;
        if (Color) query += ` AND ItemID IN (SELECT ItemID FROM ProductFilters WHERE FilterKey = 'Color' AND FilterValue = '${Color}')`;
        if (Storage) query += ` AND ItemID IN (SELECT ItemID FROM ProductFilters WHERE FilterKey = 'Storage' AND FilterValue = '${Storage}')`;
...
router.get('/category/:category', async (req, res) => {
    try {
        ...
        const [products] = await connection.query(
            `SELECT ItemID, ItemName, Description, Category, SellingPrice FROM Inventory WHERE Category = ?`, 
            [category]
        );
        res.json(products);
    } catch (error) {
